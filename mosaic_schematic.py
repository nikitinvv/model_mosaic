...
/SCALE (=8) downscaled prototype of the real experiment:
  real                    → /8 prototype (this schematic)
  detector 3232×2426 px   → 404×303 px
  sample   20480×19680 vx → 2560×2460 vx  (matches upsample_extract init)
  pixel/voxel 1.4 µm      → 11.2 µm  (physical dims stay real)
  overlaps 200 px         → 25 px

Sample cylinder ⌀3.58 mm × 3.44 mm tall (real physical size), rotating
about the vertical axis.  Reconstruction voxel = detector pixel
(upsample_big.py sets UPS=1).
...
SCALE        = 8            # downscale factor for the prototype (real → /SCALE)
PIXEL_UM     = 1.4*SCALE   # effective pixel size in the /SCALE prototype (11.2 µm)
DET_W        = 3232 // SCALE   # detector width  (x), px  =  404 (real 3232)
DET_H        = 2426 // SCALE   # detector height (y), px  =  303 (real 2426)
# Sample cylinder in the init volume (from upsample_extract.py):
#   OUT_NZ  = 2560, Z_PAD = 50   =>  SAMPLE_NZ = 2460
#   OUT_NYX = 2744, CIRCLE_DIAM  = 2560  (soft-tapered circular mask)
# upsample_big.py runs with UPS=1, so recon voxel = detector pixel.
VOXEL_UM     = PIXEL_UM                 # no upsampling
SAMPLE_D_PX  = 2560*8//SCALE                     # mask diameter in voxels
SAMPLE_H_PX  = 2460*8//SCALE                  # sample z-extent (OUT_NZ - 2·Z_PAD)
SAMPLE_D_MM  = SAMPLE_D_PX * VOXEL_UM * 1e-3      # 3.584 mm
SAMPLE_H_MM  = SAMPLE_H_PX * VOXEL_UM * 1e-3      # 3.444 mm
OVERLAP      = 200 // SCALE   # x-overlap between adjacent tiles, px  (real 200 → 25)
AXIS_INSET   = 200 // SCALE   # rotation axis inside tile 0's left border, px  (real 200 → 25)
Z_OVERLAP    = 200 // SCALE   # z-overlap between vertical positions, px  (real 200 → 25)
NTHETA       = 16           # angles over 360 deg
ANG_MAX      = 360          # scan range, deg
...
def main() -> None:
    ...
    # Angle spokes
    thetas = np.linspace(0, np.pi * ANG_MAX / 180, NTHETA, endpoint=False)
    for th in thetas:
    ...
    axR.text(0, arc_r + 0.5, f"{ANG_MAX}° / {NTHETA} angles",
